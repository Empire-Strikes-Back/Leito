(ns Leito.map-view
  (:require  [reagent.core :as r]
             ...
             [cljs.pprint :as pp]
             [re-frame.core :as rf]
             [Leito.map-subs :as subs]
             [Leito.map-evs :as evs]
             [Leito.map-core]
             [Leito.core :refer [->clj  pretty-json
                               pretty-json-str]]
  ...
  (let []
    (r/create-class
     {:display-name "ol-map"
      :component-did-mount
      (fn [this]
        (rf/dispatch [:Leito.map-core/create-olmap "map-container"]))
      ...
      (fn [this]
        (rf/dispatch [:Leito.map-core/set-olmap-target nil]))
      :reagent-render
      (fn [x y z]
...
#_(defn ol-map-layers
  []
  (let [geoserver-host (rf/subscribe [:Leito.subs/geoserver-host])
     ...
     {:display-name "ol-map-layers"
      ...
      (fn [this]
        (let [{:keys [ids geoserver-host wms-use-auth? credentials]} (r/props this)]
          (rf/dispatch [:Leito.map-core/sync-layer-ids [ids {:geoserver-host geoserver-host
...
(defn ol-map-layers
  ...
  (let [geoserver-host (rf/subscribe [:Leito.subs/geoserver-host])
        ids-ref (rf/subscribe [::subs/checked-layer-ids])
        awms-use-auth? (rf/subscribe [:Leito.settings-subs/wms-use-auth?])
        acredentials (rf/subscribe [:Leito.subs/credentials])
        ]
    (fn []
      (let [host @geoserver-host
            ids @ids-ref
       ...
       [ant-button-group {:size "default"}
        [ant-button
         {:icon "reload"
          ...
          :on-click (fn []
                      (rf/dispatch [:Leito.map-core/refetch-wms-layers]))}]]])))
     ...
     {:display-name "map-click-inner"
      ...
      (fn [this]
        (let [{:keys [on-click]} (r/props this)]
          (rf/dispatch [:Leito.map-core/mapclick-listen
                        {:on-click on-click}])))
      :component-will-unmount
      (fn [this]
        #_(js/console.log "will unmount click")
        (let [{:keys [on-click]} (r/props this)]
          (rf/dispatch [:Leito.map-core/mapclick-unlisten
                        {:on-click on-click}])))
  ...
  (let [amap-click? (rf/subscribe [::subs/wfs-search-map-click?])]
    (fn []
      (let [map-click? @amap-click?
            on-click (fn [ev]
                       (rf/dispatch [:Leito.map-core/wfs-search-mapclick [ev]]))]
        (when map-click?
     ...
     {:display-name "wfs-search-area-box-inner"
      ...
      (fn [this]
        (let [{:keys [on-draw-end]} (r/props this)]
          (rf/dispatch [:Leito.map-core/wfs-search-mapbox-listen
                        {:on-draw-end on-draw-end}])))
      :component-will-unmount
      (fn [this]
        (rf/dispatch [:Leito.map-core/wfs-search-mapbox-unlisten]))
      :reagent-render (fn [] nil)})))
  ...
  (let [aactive? (rf/subscribe [::subs/wfs-search-area-box?])]
    (fn []
      (let [active? @aactive?
            on-draw-end (fn [ev]
                          (rf/dispatch
                           [:Leito.map-core/wfs-search-mapbox [ev]]))]
        (when active?
  ...
  (let [amap-click? (rf/subscribe [::subs/modify-wfs-click?])]
    (fn []
      (let [map-click? @amap-click?
            on-click
            (fn [ev]
              (rf/dispatch [:Leito.map-core/wfs-modify-mapclick [ev]]))]
        (when map-click?
          [map-click-inner {:on-click on-click}])))))
     ...
     {:display-name "modify-sync-ol-features-inner"
      ...
      (fn [this]
        (let [{:keys [features]} (r/props this)]
          (rf/dispatch [:Leito.map-core/sync-modified-features {:features features}])))
      :component-did-update
      (fn [this]
        (let [{:keys [features]} (r/props this)]
          (rf/dispatch [:Leito.map-core/sync-modified-features {:features features}])))
     ...
     {:display-name "modify-sync-modify-interactions-inner"
      ...
      (fn [this]
        (let [{:keys []} (r/props this)]
          (rf/dispatch [:Leito.map-core/add-modify-interactions {}])))
      :component-will-unmount
      (fn [this]
        (let [{:keys []} (r/props this)]
          (rf/dispatch [:Leito.map-core/remove-modify-interactions {}])))
      :reagent-render
      (fn []
        nil)})))
...
(defn panel []
  (let [module-count @(rf/subscribe [::subs/module-count])
        base-url @(rf/subscribe [:Leito.subs/base-url])
        ]
