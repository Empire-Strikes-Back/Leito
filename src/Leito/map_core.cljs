 ...
 (fn-traced
  ...
  (do
    (let [[ev] ea
          ...
          geom-name (:Leito.db.settings/geometry-name db)
      ...
      {:dispatch [:Leito.map-evs/wfs-search {:filter filter}]}))))
 ...
 (fn-traced
  ...
  (do
    (let [[ev] ea
      ...
      {:dispatch [:Leito.map-evs/modify-wfs-click {:filter filter}]}))))
...
(rf/reg-cofx
 ...
 (fn [{:keys [db] :as cofx} [k]]
   (let [geom-name (:Leito.db.settings/geometry-name db)]
