...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db stored-db] :as coef} av]
            {:db (merge
                  Leito.db/default-db
                  stored-db
                  {:Leito.db.core/profiles
                   (merge
                    (:Leito.db.core/profiles Leito.db/default-db)
                    (:Leito.db.core/profiles stored-db))
                   :Leito.db.core/active-profile-key
                   (or
                    (:Leito.db.core/active-profile-key stored-db)
                    (:Leito.db.core/active-profile-key Leito.db/default-db))}
                  )}))

(rf/reg-event-db
 ::set-active-panel
 (fn-traced [db [_ active-panel]]
            (assoc db :Leito.db.core/active-panel active-panel)))
...
(rf/reg-event-db
 ::inc-module-count
 (fn-traced [db [_ active-panel]]
            (let [kw :Leito.db.core/module-count]
              (assoc db kw (inc (kw db))))))

(rf/reg-event-fx
 ::apply-server-settings
 (fn-traced [{:keys [db]} [_ ea]]
            (let [apk (:Leito.db.core/active-profile-key db)
                  proxy-host (get-in db [:Leito.db.core/profiles apk :proxy-host])
                  proxy-path (:Leito.db.core/proxy-path db)
                  body (str  {:proxy-geoserver-host proxy-host
                              :proxy-path proxy-path})]
              {:dispatch [:Leito.evs/request
                          {:method :post
                           :body body
...
;  [(re-frame/)]
 ...
 (fn-traced [{:keys [db event] :as ctx} [_ ea]]
            (let [base-url (get-in db [:Leito.db.core/api :base-url])
                  ...
                  apk (:Leito.db.core/active-profile-key db)
                  uname (get-in db [:Leito.db.core/profiles apk :username])
                  pass (get-in db [:Leito.db.core/profiles apk :password])]
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  {:db (update db :Leito.db.core/log-que conj
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  {:db (assoc db :Leito.db.core/log-que [])}))
 ...
 (fn [{:keys [db] :as cofx} ea]
   (let [stored-db  (storage/read-db)
     ...
     (assoc cofx :stored-db
            (merge db
                   profile-db
                   {:Leito.db.core/profiles profiles
                    :Leito.db.core/active-profile-key apk})))))
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [pfs (:Leito.db.core/profiles db)
        ...
        pf {:key k
            ...
            :username "admin"
            :password "geoserver"}
        pfs (assoc (:Leito.db.core/profiles db) k pf)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc db :Leito.db.core/profiles pfs)})))
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [k (aget ea "key")
        pfs (dissoc (:Leito.db.core/profiles db) k)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc db :Leito.db.core/profiles pfs)})))
...
(rf/reg-event-fx
 ...
 (fn-traced
  ...
  (let [apk (aget ea "key")
    ...
    {:db (merge
          db
          (or profile-db Leito.db/default-db)
          {:Leito.db.core/active-profile-key apk
           :Leito.db.core/profiles (merge (:Leito.db.core/profiles db) profiles)})
     :dispatch [:Leito.evs/apply-server-settings]})))

(rf/reg-event-fx
 ::update-profiles
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [pfs (deep-merge (:Leito.db.core/profiles db) ea)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc-in db [:Leito.db.core/profiles] pfs)
