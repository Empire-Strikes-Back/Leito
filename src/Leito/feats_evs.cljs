(ns Leito.feats-evs
  (:require [re-frame.core :as rf]
            ...
            [Leito.core :refer [url-search-params]]
            [Leito.wfs :refer [attrs->eqcl-ilike]]
...
(rf/reg-event-fx
 ::search
 [(rf/inject-cofx :Leito.feats-core/get-editor-val [:ecql])]
 (fn-traced [{:keys [db get-editor-val]} [_ ea]]
   (let [ftype-input (:Leito.db.feats/feature-type-input db)
         [fpref ftype] (try (str/split ftype-input \:)
                            (catch js/Error e
                              (do (js/console.warn e)
                                  ["undefined:undefined"])))
         input (:Leito.db.feats/search-input db)
         s (or (:input ea) input)
         table-mdata (:Leito.db.feats/search-table-mdata db)
         total (get-in db [:Leito.db.feats/search-res :total])
         pag (:pagination table-mdata)
         use-eqcl-filter? (:Leito.db.feats/use-eqcl-filter? db)
         ...
         selected-attrs (:Leito.db.feats/selected-attrs db)
         attrs (get-in db [:Leito.db.feats/fetch-ftype-mdata-layer-res
     ...
     #_{:dispatch-n (list
                 [:Leito.evs/request
                  {:method :post
                   ...
                   :on-success [::search-res]
                   :on-failure [::search-res]}]
                 [:Leito.feats-core/set-editor-xml [:request body]])
      :db (merge db {:Leito.db.feats/search-input s
                     :Leito.db.feats/search-table-mdata
                     (if (:input ea)
...
(rf/reg-event-db
 ::search-res
 (fn-traced [db [_ ea]]
            (assoc db :Leito.db.feats/search-res ea)))

(rf/reg-event-fx
 ::tx-feature
 [(rf/inject-cofx :Leito.feats-core/get-editor-val [:data])]
 (fn-traced [{:keys [db get-editor-val]} [_ ea]]
            (let [ftype-input (:Leito.db.feats/feature-type-input db)
                  [fpref ftype] (try (str/split ftype-input \:)
                                     (catch js/Error e
                                       (do (js/console.warn e)
                                           ["undefined:undefined"])))
                  fns (get-in db [:Leito.db.feats/fetch-ftype-mdata-ns-res
                                  :namespace :uri])
                  tx-type (:tx-type ea)
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ id ea]]
            {:db (assoc db :Leito.db.feats/tx-res ea)
             :dispatch-n (list
...
(rf/reg-event-fx
 ::search-input
 (fn-traced [{:keys [db]} [_ ea]]
            (let [k :Leito.db.feats/search-input
                  v ea]
              {:db (assoc db k v)
               })))
...
(rf/reg-event-fx
 ::search-table-mdata
 (fn-traced [{:keys [db]} [_ ea]]
   (let [key :Leito.db.feats/search-table-mdata]
     {:dispatch [:Leito.feats-evs/search {}]
      :db (assoc db key ea)})))

(rf/reg-event-fx
 ::select-feature
 (fn-traced [{:keys [db]} [_ ea]]
            (let [key :Leito.db.feats/select-feature]
              {:db (assoc db key ea)
               :dispatch [:Leito.feats-core/set-editor-json [:data ea]]})))

(rf/reg-event-fx
 ::feature-type-input
 (fn-traced [{:keys [db]} [_ ea]]
   (let [v ea
         k :Leito.db.feats/feature-type-input]
     {:db (assoc db k v)
...
(rf/reg-event-fx
 ::feature-ns
 (fn-traced [{:keys [db]} [_ ea]]
            (let [k :Leito.db.feats/feature-ns]
              {:db (assoc db k ea)
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ ea]]
            {:db (assoc db :Leito.db.feats/fetch-ftype-mdata-layer-res ea)}))

(rf/reg-event-fx
 ::fetch-ftype-mdata-ns-res
 (fn-traced [{:keys [db]} [_ ea]]
            {:db (assoc db :Leito.db.feats/fetch-ftype-mdata-ns-res ea)}))
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ ea]]
            (let [v ea]
              {:db (assoc db :Leito.db.feats/selected-attrs v)})
            ))
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ ea]]
            (let [v ea]
              {:db (assoc db :Leito.db.feats/use-eqcl-filter? v)})))