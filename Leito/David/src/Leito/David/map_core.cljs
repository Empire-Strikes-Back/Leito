(ns Leito.David.map-core
  (:require  [reagent.core :as r]
             [cljs.repl :as repl]
             [cljs.pprint :as pp]
             [re-frame.core :as rf]
             [day8.re-frame.tracing :refer-macros [fn-traced defn-traced]]
             [Leito.David.map-ol :as ol]
 ...
 (fn-traced
  ...
  (do
    (let [[ev] ea
          ...
          geom-name (:Leito.David.db.settings/geometry-name db)
      ...
      {:dispatch [:Leito.David.map-evs/wfs-search {:filter filter}]}))))
 ...
 (fn-traced
  ...
  (do
    (let [[ev] ea
      ...
      {:dispatch [:Leito.David.map-evs/modify-wfs-click {:filter filter}]}))))
...
(rf/reg-cofx
 ...
 (fn [{:keys [db] :as cofx} [k]]
   (let [geom-name (:Leito.David.db.settings/geometry-name db)]
