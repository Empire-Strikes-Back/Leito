(ns Leito.David.feats-view
  (:require  [reagent.core :as r]
             [cljs.repl :as repl]
             [cljs.pprint :as pp]
             [re-frame.core :as rf]
             [Leito.David.feats-subs :as subs]
             [Leito.David.feats-evs :as evs]
             [Leito.David.feats-core]
             ...
             #_[Leito.David.core.extra :refer [extra-component]]))
  ...
  (let [default-value ""
    ...
    (fn []
      [react-ace {:name "editor-feature"
                  ...
                  :value @av
                  :on-load (fn [ref]
                             (rf/dispatch [:Leito.David.feats-core/set-editor-preserve [:data ref]]))
  ...
  (let [default-value ""
    ...
    (fn []
      [react-ace {:name "editor-response"
                  ...
                  :value @av
                  :on-load (fn [ref]
                             (rf/dispatch [:Leito.David.feats-core/set-editor-preserve [:response ref]]))
                  :on-change (fn [v ev] (reset! av v))
                  :editor-props {"$blockScrolling" js/Infinity}}])))
  ...
  (let [default-value ""
    ...
    (fn []
      [react-ace {:name "editor-request"
                  ...
                  :value @av
                  :on-load (fn [ref]
                             (rf/dispatch [:Leito.David.feats-core/set-editor-preserve [:request ref]]))
                  :on-change (fn [v ev] (reset! av v))
                  :editor-props {"$blockScrolling" js/Infinity}}])))
  ...
  (let [default-value
    ...
    (fn []
      [react-ace {:name "editor-ecql"
                  ...
                  :value @av
                  :on-load (fn [ref]
                             (rf/dispatch [:Leito.David.feats-core/set-editor-preserve [:ecql ref]]))
                  :on-change (fn [v ev] (reset! av v))
                  :editor-props {"$blockScrolling" js/Infinity}}])))
