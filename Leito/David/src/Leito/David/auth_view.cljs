(ns Leito.David.auth-view
  (:require  [reagent.core :as r]
             [re-frame.core :as rf]
             [cljs.repl :as repl]
             [cljs.pprint :as pp]
             [Leito.David.auth-evs :as evs]
             [Leito.David.auth-subs :as subs]
   ...
   {:title ""
    ...
    (fn [txt rec idx]
      (let [on-activate (fn [ea]
                          (let []
                            (rf/dispatch
                             [:Leito.David.evs/activate-profile rec])))
            on-remove (fn [ea]
                        (let []
                          (rf/dispatch
                           [:Leito.David.evs/remove-profile rec])))]
        (r/as-element
...
(defn profiles-table
  []
  (let [adata (rf/subscribe [:Leito.David.subs/profiles])
        aapk (rf/subscribe [:Leito.David.subs/active-profile-key])
        refs (atom {})]
    (fn []
        ...
        [ant-table {:show-header true
                    ...
                    :title (fn [_]
                             (r/as-element
                              [:section
                               [ant-button
                                {:on-click #(rf/dispatch [:Leito.David.evs/add-profile])
                               ...
                               [ant-button
                                {:on-click
                                 (fn []
                                   (rf/dispatch
                                    [:Leito.David.evs/update-profiles (cell-refs->data @refs)]))
...
(defn profile-form
  []
  (let [apf (rf/subscribe [:Leito.David.subs/active-profile])
        uname (r/atom nil)
        pass (r/atom nil)]
    (fn []
