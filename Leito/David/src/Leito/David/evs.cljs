(ns Leito.David.evs
  (:require
   ...
   [Leito.David.db :refer [default-db]]
   #_[vimsical.re-frame.cofx.inject :as inject]
   [ajax.core :as ajax]
   [ajax.edn]
   [day8.re-frame.tracing :refer-macros [fn-traced defn-traced]]
   [Leito.David.core :refer [basic-creds]]
   ["antd/lib/message" :default AntMessage]
   [Leito.David.storage]
   [Leito.David.core :refer  [deep-merge]]
   [Leito.David.storage :as storage]))
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db stored-db] :as coef} av]
            {:db (merge
                  Leito.David.db/default-db
                  stored-db
                  {:Leito.David.db.core/profiles
                   (merge
                    (:Leito.David.db.core/profiles Leito.David.db/default-db)
                    (:Leito.David.db.core/profiles stored-db))
                   :Leito.David.db.core/active-profile-key
                   (or
                    (:Leito.David.db.core/active-profile-key stored-db)
                    (:Leito.David.db.core/active-profile-key Leito.David.db/default-db))}
                  )}))

(rf/reg-event-db
 ::set-active-panel
 (fn-traced [db [_ active-panel]]
            (assoc db :Leito.David.db.core/active-panel active-panel)))
...
(rf/reg-event-db
 ::inc-module-count
 (fn-traced [db [_ active-panel]]
            (let [kw :Leito.David.db.core/module-count]
...
(rf/reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ ea]]
            (let [apk (:Leito.David.db.core/active-profile-key db)
                  proxy-host (get-in db [:Leito.David.db.core/profiles apk :proxy-host])
                  proxy-path (:Leito.David.db.core/proxy-path db)
                  body (str  {:proxy-geoserver-host proxy-host
                              :proxy-path proxy-path})]
              {:dispatch [:Leito.David.evs/request
                          {:method :post
                           :body body
...
;  [(re-frame/)]
 ...
 (fn-traced [{:keys [db event] :as ctx} [_ ea]]
            (let [base-url (get-in db [:Leito.David.db.core/api :base-url])
                  ...
                  apk (:Leito.David.db.core/active-profile-key db)
                  uname (get-in db [:Leito.David.db.core/profiles apk :username])
                  pass (get-in db [:Leito.David.db.core/profiles apk :password])]
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  {:db (update db :Leito.David.db.core/log-que conj
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  {:db (assoc db :Leito.David.db.core/log-que [])}))
 ...
 (fn [{:keys [db] :as cofx} ea]
   (let [stored-db  (storage/read-db)
     ...
     (assoc cofx :stored-db
            (merge db
                   profile-db
                   {:Leito.David.db.core/profiles profiles
                    :Leito.David.db.core/active-profile-key apk})))))
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [pfs (:Leito.David.db.core/profiles db)
        ...
        pf {:key k
            ...
            :username "admin"
            :password "geoserver"}
        pfs (assoc (:Leito.David.db.core/profiles db) k pf)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc db :Leito.David.db.core/profiles pfs)})))
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [k (aget ea "key")
        pfs (dissoc (:Leito.David.db.core/profiles db) k)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc db :Leito.David.db.core/profiles pfs)})))
...
(rf/reg-event-fx
 ...
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [k (:key ea)
        v (:Leito.David.db.core/profiles db)
...
(rf/reg-event-fx
 ...
 (fn-traced
  ...
  (let [apk (aget ea "key")
    ...
    {:db (merge
          db
          (or profile-db Leito.David.db/default-db)
          {:Leito.David.db.core/active-profile-key apk
           :Leito.David.db.core/profiles (merge (:Leito.David.db.core/profiles db) profiles)})
     :dispatch [:Leito.David.evs/apply-server-settings]})))

(rf/reg-event-fx
 ::update-profiles
 (fn-traced
  [{:keys [db]} [_ ea]]
  (let [pfs (deep-merge (:Leito.David.db.core/profiles db) ea)]
    (do
      (storage/assoc-in-store! [:profiles] pfs))
    {:db (assoc-in db [:Leito.David.db.core/profiles] pfs)
