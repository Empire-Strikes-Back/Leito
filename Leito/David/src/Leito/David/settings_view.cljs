(ns Leito.David.settings-view
  (:require  [reagent.core :as r]
             [re-frame.core :as rf]
             [cljs.repl :as repl]
             [cljs.pprint :as pp]
             [Leito.David.settings-evs :as evs]
             [Leito.David.settings-subs :as subs]
         ...
         #_[ant-row
            ...
            [ant-col {:span 8}
             [ant-input {:value @proxy-path
                         :on-change
                         #(rf/dispatch [:Leito.David.settings.events/set
                                        :Leito.David.db.core/proxy-path
                                        (.. % -target -value)])}]]
            [ant-col {:span 4}
             [ant-button
              {:on-click (fn [] (rf/dispatch [:Leito.David.evs/apply-server-settings]))}
              "apply"]]]
         [:br]
         [ant-row
          ...
          [ant-col {:span 8}
           [ant-checkbox {:size "small"
                          ...
                          :on-change
                          #(rf/dispatch [::evs/set
                                         :Leito.David.db.settings/wms-use-auth?
                                         (.. % -target -checked)])}]]]
         [:br]
         [ant-row
          ...
          [ant-col {:span 8}
           [ant-input {:size "small"
                       ...
                       :on-change
                       #(rf/dispatch [::evs/set
                                      :Leito.David.db.settings/geometry-name
                                      (.. % -target -value)])}]]]
         [:br]
         ;
         ]
