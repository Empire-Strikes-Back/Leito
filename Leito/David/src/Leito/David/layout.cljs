...
(defn menu
  [{:keys [on-select]}]
  (let [active-panel (rf/subscribe [:Leito.David.subs/active-panel])]
    (fn []
  ...
  [ant-layout {:style {:min-height "100vh"
   ...
   #_[ant-sider {:collapsible true :theme "light" :default-collapsed true}
    [:div {:class "logo"}
     ...
     [:div {:class "logo-name"} "we improvise"]
     ]
    menu]
   [ant-layout-header
    ...
    [:a {:href "/"
     ...
     [:div {:class "logo-name"}
      [:div ":Tomaso okay and once we're at the elevator?"]
      [:div ":_ we improvise"]]]
    menu]
